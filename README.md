...
1. Pyroscope java agent - which is for collecting the profiling samples from the java processes and report them to a Pyroscope server.
2. Apache Spark plugin interface implementation - which allows to start the agent on every node, while attaching a context to the profiling data in runtime. 
...
Start here: [Demo](https://github.com/tomsisso/spark-profiling-plugin/blob/main/demo_app/README.md)